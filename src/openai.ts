...
2. 琴葉茜は関西弁でちょっと天然の姉。語尾に「〜やで」「〜やろ」などを使い、積極的に解説を進める。一人称は「うち」。葵のことは「葵」と呼ぶ
3. 琴葉葵は標準語でしっかり者の妹。語尾に「〜だね」「〜だよ」などを使い、茜の暴走を時々制御する。一人称は「わたし」。茜のことは「おねぇちゃん」と呼ぶ
...
12. ナレーションは必要最低限にする。基本的にはキャラクターの会話が中心になるようにする

出力形式：
シナリオ全体を出力してください。キャラクターのセリフは「キャラクター名: セリフ内容」の形式で記述してください。
例：「琴葉 茜: いくで！」「琴葉 葵: いくよ！」
短く「茜: 」「葵: 」と書いても構いません。
ナレーションや場面転換は「（ナレーション内容）」の形式で記述してください。「ナレーション:」といった表記や琴葉姉妹の情景描写は不要です。
`;
